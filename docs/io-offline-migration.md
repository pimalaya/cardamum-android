...
- Delta enumerates landed for all four backends (stage 6 complete): io-jmap (ContactCard/changes) and io-people (sync tokens) already had the wire primitives; io-msgraph gained a contacts delta coroutine (rows flagged @removed, deltaLink checkpoint, HTTP 410 as the expiry signal) and rides as a local path patch alongside io-offline until pushed.
