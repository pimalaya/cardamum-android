...
### Stage 6: one form for edits and conflicts (landed 2026-07-14)

The conflict form stopped being a reduced page: opening a diverged contact now renders the exact same editor as a plain edit, every filled field showing and the add-field controls (bar button, section headers) staying live. The differences narrowed to two signals: each disagreeing row wears the error-tinted diverged glyph until opened and confirmed with OK (rows added during the session never wear it), and while any glyph remains the save disc turns the error tone and carries the same warning glyph instead of a dimmed check, refusing to validate. Review keys moved from page positions to field identities (single-field paths, changed-list section plus loaded position), so editing, adding and removing around the conflicts behaves exactly like a normal edit without misplacing the warnings. A row only force-renders while its divergence awaits review; once reviewed it follows the plain visibility rules, so clearing a value drops its line like in a normal edit.
